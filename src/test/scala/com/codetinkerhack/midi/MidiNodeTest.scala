...
class MidiNodeTest extends AnyFlatSpec with should.Matchers {
  ...
  it should "send message via Output channel 1 of Node A to Input channel 1 of Node B" in {
    ...
    chain.send(new MidiMessageContainer(new ShortMessage(NOTE_ON, 1, 1, 1)), 0L, null)

    val message = testNode.getLastMessage()
    message._1.asInstanceOf[MidiMessageContainer] should matchMessage(new MidiMessageContainer(new ShortMessage(NOTE_ON, 1, 1, 1)))
    message._2 should equal (0L)
  }
  ...
  it should "route message via Output channel 1 of Node A to Input channel 2 of Node B" in {
    ...
    val message = testNode.getLastMessage()
    message._1.asInstanceOf[MidiMessageContainer] should matchMessage(new MidiMessageContainer(new ShortMessage(NOTE_ON, 2, 1, 1)))
    message._2 should equal (0L)
  }
  ...
  it should "transmit message only on specific channel output matching message channel" in {
    ...
    MidiParallel(chain, chain1).send(new MidiMessageContainer(new ShortMessage(NOTE_ON, 1, 1, 1)), 0L, null)
